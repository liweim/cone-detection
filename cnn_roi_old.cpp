...
void forwardDetectionORB(const std::string& imgPath){
  ...
  for(size_t i = 0; i < keypoints.size(); i++){
    ...
    if(point3D.y>0.5 && point3D.y<0.9){
  ...
  if(inputs.size()>0){
    ...
    std::vector <cv::Point> cones = imRegionalMax(probMap, 20, threshold, 20);
    ...
    for(size_t i = 0; i < cones.size(); i++){
      ...
      if(ANNOTATE){
        std::string path = imgPath.substr(0,index-7)+"/annotations/"+std::to_string(maxIndex);
...
int main( int argc, char** argv )
{
  ...
  forwardDetectionORB("data/skidpad1/images/25.png");
	// for(int i = 0; i <= 315; i++){
 //    int rand = std::rand()%10;
 //    if(rand < 10){
 //  		auto startTime = std::chrono::system_clock::now();
 //  		forwardDetectionORB("data/skidpad1/images/"+std::to_string(i)+".png");
