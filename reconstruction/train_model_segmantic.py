...
def load_data(cone_id, train_num):
    images = []
    classes = []
    imgs = []
    masks = []
    ...
    annotation_folder_path = join('annotations', cone_id)
    image_folder_path = join('images', cone_id)
    annotation_paths = os.listdir(annotation_folder_path)
    for img_path in annotation_paths:
        img = cv2.imread(join(image_folder_path, img_path))
        mask = cv2.imread(join(annotation_folder_path, img_path), 0)
        imgs.append(img)
        masks.append(mask)
    ...
    while num_back < train_num or num_front < train_num:
        ...
        if np.max(image) > 0:
            if mask[r, c] > 0 and num_front < train_num:
                images.append(image)
                classes.append(1)
                num_front += 1
            if mask[r, c] == 0 and num_back < train_num:
                images.append(image)
                classes.append(0)
                num_back += 1
...
def train_model(cone_id, train_num):
    save_path = join('models', 'model_cone')
    batch_size = 256
    x_train, x_test, y_train, y_test = load_data(cone_id, train_num)
...
if __name__ == '__main__':
    ...
    train_model(cone_id = args.cone_id, train_num = args.train_num)
